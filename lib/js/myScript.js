	...
	success: function(result, textStatus, xhr){
		...
		if (xhr.status === 200) {
			...
			for(var i = 0; i < result['features'].length;i++) {
				...
				optiontag.value = result['features'][i]['properties']['ADMIN'] + " (" + result['features'][i]['properties']['ISO_A3']+")";
				optiontag.innerText = result['features'][i]['properties']['ADMIN'] + " (" + result['features'][i]['properties']['ISO_A3']+")";

				if(optiontag.innerText === "United Kingdom (GBR)") {
	...
	success: function(result, textStatus, xhr){
		...
		document.getElementById("countryinformationdetailed").removeChild(custompara2);
          ...
          if(result['data']['type'] === "Polygon") {
			for(var i = 0; i < result['data']['coordinates'][0].length;i++) {
				var tmp = result['data']['coordinates'][0][i][0];
				result['data']['coordinates'][0][i][0] = result['data']['coordinates'][0][i][1];
				result['data']['coordinates'][0][i][1] = tmp;
			}
	
					   custompolygon.push(new L.polygon(result['data']['coordinates'][0]));
					   mymap.addLayer(custompolygon[0]);
					   custompolygon[0].addTo(mymap);
					   	  mymap.setView(result['data']['coordinates'][0][0],6);
		...
		document.getElementById("countryinformationdetailed").appendChild(custompara2);
			...
			else if(result['data']['type'] === "MultiPolygon") {
for(var i = 0; i < result['data']['coordinates'].length;i++) {
for(var j = 0; j < result['data']['coordinates'][i][0].length;j++) {
				var tmp = result['data']['coordinates'][i][0][j][0];
				result['data']['coordinates'][i][0][j][0] = result['data']['coordinates'][i][0][j][1];
				result['data']['coordinates'][i][0][j][1] = tmp;
			...
			for (var i = 0; i < result['data']['coordinates'].length; i++) {	
		    custompolygon2.push(new L.polygon(result['data']['coordinates'][i][0]));
			mymap.addLayer(custompolygon2[i]);
		    custompolygon2[i].addTo(mymap);	

		}
		var mytmp = Math.ceil((result['data']['coordinates'].length /2)-1);
		console.log(result['data']['coordinates'].length);
				console.log(mytmp);

		 mymap.setView(result['data']['coordinates'][mytmp][0][0],6);
...
if(document.getElementById("countryinformationdetailed").contains(custompara2) && document.getElementById("countryinformationdetailed").contains(custombrace) ) {
   ...
   $.ajax({
	url: "lib/php/countries_small.php",
	type: "POST",
	dataType: "json",
	data: {
		countryname: countryname2
  ...
  $.ajax({
	...
	success: function(result, textStatus, xhr){
		...
		if (xhr.status === 200) {
			...
			document.getElementById("Capital1").innerHTML = "Capital: " + result['data'][0]['capital'];	
			document.getElementById("population1").innerHTML = "Population: " + numberWithCommas(result['data'][0]['population']);
				document.getElementById("countryflag").setAttribute("src",result['data'][0]['flag']);
		
		
			for(var i =0;i<result['data'][0]['currencies'].length;i++){
				if(i <= 0) {
			if(result['data'][0]['currencies'][i]['name'] != null && result['data'][0]['currencies'][i]['name'] != undefined) {
					document.getElementById("Currency1").innerHTML = "Currency: "+result['data'][0]['currencies'][i]['name'];
				}
			}
				else {
				if(result['data'][0]['currencies'][i]['name'] != null && result['data'][0]['currencies'][i]['name'] != undefined) {
					document.getElementById("Currency1").innerHTML = document.getElementById("Currency1").innerHTML +", "+result['data'][0]['currencies'][i]['name'];
				}
			}
			}
            for(var i =0;i<result['data'][0]['languages'].length;i++){
				if(i <= 0) {
				if(result['data'][0]['languages'][i]['name'] != null && result['data'][0]['languages'][i]['name'] != undefined) {
					document.getElementById("Language1").innerHTML = "Language: "+result['data'][0]['languages'][i]['name'];
				}
				//else no data found!
			}
				else {
				if(result['data'][0]['languages'][i]['name'] != null && result['data'][0]['languages'][i]['name'] != undefined) {
					document.getElementById("Language1").innerHTML = document.getElementById("Language1").innerHTML +", "+result['data'][0]['languages'][i]['name'];
				}
			}
			}
			   for(var i =0;i<result['data'][0]['callingCodes'].length;i++){
				if(i <= 0) {
					if(result['data'][0]['callingCodes'][i] != null && result['data'][0]['callingCodes'][i] != undefined) {
					document.getElementById("CallingCode1").innerHTML = "Calling code: +"+result['data'][0]['callingCodes'][i];
				}
				}
				else {
					if(result['data'][0]['callingCodes'][i] != null && result['data'][0]['callingCodes'][i] != undefined) {
					document.getElementById("CallingCode1").innerHTML = document.getElementById("CallingCode1").innerHTML +", +"+result['data'][0]['callingCodes'][i];
				}
			}
			}
		}
