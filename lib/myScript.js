	...
	success: function(result, textStatus, xhr){
		...
		if (xhr.status === 200) {
			...
			for(var i = 0; i < result['data'].length;i++) {
				...
				anchortag.setAttribute("onclick","onClickLink(this)");
				anchortag.innerText = result['data'][i]['properties']['ADMIN'] + " (" + result['data'][i]['properties']['ISO_A3']+")";
				customdiv44.appendChild(anchortag);
			}
			...
			for(var i = 0; i < result['data']['coordinates'][0].length;i++) {
				...
				result['data']['coordinates'][0][i][1] = tmp;
					   ...
					   custompolygon[0].addTo(mymap);
					   	  mymap.setView(result['data']['coordinates'][0][0],7);
...
for(var j = 0; j < result['data']['coordinates'][i][0].length;j++) {
		 ...
		 mymap.setView(result['data']['coordinates'][0][0][0],7);
		...
		if (xhr.status === 200) {
            ...
            for(var i =0;i<result['data'][0]['languages'].length;i++){
			   ...
			   for(var i =0;i<result['data'][0]['callingCodes'].length;i++){
				...
				document.getElementById("countryflag").setAttribute("src","lib/noflag.png");
